...
export function PageData() {
  ...
  if (isLoading)
    return (
      <div className="absolute top-0 left-0 h-screen w-screen bg-[#f85606]">
        <Image
          className={`${styles.bounce} absolute top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%]`}
  ...
  const cards = data
    .filter((items) => {
      return searchKeyword.toLowerCase() === " "
        ? items
        : items.title.toLowerCase().includes(searchKeyword);
    })
    .map((items) => {
      return (
        <Link key={items.id} href={`/${items.id}`}>
          <div className="flex flex-col md:flex-row items-center shadow-xl rounded mb-5 w-full h-full gap-5">
            ...
            <div className="flex flex-col gap-3 items-center md:items-start">
              <h2 className="font-semibold line-clamp-2 w-[80%]">
                {items.title}
              </h2>
              ...
              <div className="w-24 text-center">
                <p className={`border-2 px-2 py-1 rounded-lg text-[#f85606]`}>
                  ${items.price}
                </p>
              </div>
            </div>
  ...
  return (
    <Layout>
      ...
      <div className="grid grid-cols-1 gap-5 mt-5">{cards}</div>
    </Layout>
  );
}
